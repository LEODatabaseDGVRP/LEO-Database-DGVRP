...
function setupSession(app: Express) {
  ...
  app.use(session({
    store: new memoryStore({
      ...
      stale: true // Allow stale sessions to be returned
    }),
    secret: process.env.SESSION_SECRET || 'law-enforcement-secret-key-change-in-production',
    resave: false, // Don't save session if unmodified - important for persistence
    saveUninitialized: false, // Don't save empty sessions
    rolling: false, // Don't reset expiration on each request to maintain same session
    cookie: {
      secure: false, // Disable secure flag to allow HTTP in development
      httpOnly: true,
      maxAge: 2 * 60 * 60 * 1000, // 2 hours for better persistence
      sameSite: 'lax' // Use lax for better compatibility
    },
    name: 'connect.sid' // Use default session name for compatibility
    // Remove custom genid - let express-session handle session ID generation and matching
  }));
}
