...
export default function SignupPage() {
  ...
  // Check Discord verification status on component mount and when page becomes visible
  useEffect(() => {
    const checkDiscordStatus = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.log("Discord verification status:", data);
          if (data.verified) {
            setDiscordVerified(true);
            setDiscordUser(data.user);
            console.log("Discord user set:", data.user);
          } else {
        ...
        console.log("Failed to check Discord status:", error);
      }
    };

    checkDiscordStatus();

    // Also check when the page becomes visible (e.g., after Discord redirect)
    ...
    return () => {
      ...
      window.removeEventListener('focus', checkDiscordStatus);
    };
  }, []);
  ...
  const discordAuthMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        ...
        description: error.message || "Failed to start Discord authentication. Please ensure your Discord email is verified.",
        variant: "destructive",
      });
    },
  });
              ...
              <div className="space-y-4">
                <div className="text-center">
                  ...
                  <p className="text-gray-400 text-xs mt-2">
                    Requirements: Discord email must be verified, you must be logged into Discord, and be a member of the required server with proper role permissions.
                  </p>
                </div>
              </div>
